...
static const uint32_t param_boot_flag = 0xFF0000FF; // BOOT 参数表标志 +0
static const uint32_t param_app_flag =  0xFFF000F0; // APP 参数表标志  +0
static const uint32_t quality_flag =    0xFFFF0000; // 质检            +4
#define  param_quality_offset       4               // 质检标志偏移
#define  param_addr_offset          8               // 参数表偏移
#define  param_iteration            64
static const uint32_t param_key[4]={0x01E00304, 0x05D00708, 0x09602B0C, 0x0F10A000};
 ...
 * 参数表的分区管理如下:
 * Table0    |-----------------| start
 * uint32_t 0| boot_flag       | 0000H
 *          1| quality_flag    | 0004H
 *          2|                 | 0008H
 *          3|                 | 000CH
 *        ...| Ini0            | ...H
 *         64|                 | 00FCH
 * Table1    |-----------------| start
 * uint32_t 0| boot_flag       | 0100H
 *          1| quality_flag    | 0004H
 *          2|                 | 0008H
 *          3|                 | 000CH
 *        ...| Ini1            | ...H
 ...
 * Table3 ...|-Ini3 ...--------| start
 * Table63   |-----------------| start
 * uint32_t 0| boot_flag       | 3F00H
 *          1| quality_flag    | 3F04H
 *          2|                 | 3F08H
 *          3|                 | 3F0CH
 *        ...| Ini63           | ...H
 *         64|                 | 3FFCH
...
// 搜索参数表中的空白区域
static uint16_t ParamTable_search_empty(void)
{
	uint16_t addr; // 表内地址
	uint16_t _search;
...
// 搜索最新的参数表位置
static uint16_t ParamTable_search_boot(void)
{
	uint16_t addr;      // 表内地址
...
// 搜索最新的 App参数表位置
static uint16_t ParamTable_search_app(void)
{
	uint16_t addr;      // 表内地址
	uint16_t addr_last; // 表内地址
	const uint32_t* param = NULL;
...
{
	return __ParamTable_Size-param_addr_offset;
}
// Boot 参数表
...
int ParamTable_Read(void *const Param, const int _quality, const uint32_t _size)
{
	...
	if(1==_quality)  // 写入质检标志
	{
		...
		Flash_Write_Force(param_table_start+addr+param_quality_offset, &quality_flag, 1);  // 32位写入
	}
	if(size>ParamTable_Size()) size=ParamTable_Size();
	//memcpy(Param, param_buf+4, size);
	// 读取整个表
	memcpy(param_buf, (const char*)(param_table_start+addr), __ParamTable_Size);
	// TEA 解密数据
	tea_decrypt(param_buf+param_addr_offset, ParamTable_Size(), param_key, param_iteration);
	memcpy(Param, (param_buf+param_addr_offset), size);
	return size;
}

// App 参数表
int ParamTable_Write_app(const void *const Param, const uint32_t _size)
{
	uint32_t size = _size;
	uint16_t addr; // 表内地址
	// 查找空白区域
	...
	//app_debug("[%s--%d] addr:0x%04X \r\n", __func__, __LINE__, addr);
	// 无空闲空间,操作失败
	if(addr>=param_table_size) return 0;
	if(size>ParamTable_Size()) size=ParamTable_Size();
	//app_debug("[%s--%d] addr:0x%04X size:%d \r\n", __func__, __LINE__, addr, size);
	memset(param_buf, 0xFF, sizeof(param_buf));
	memcpy(param_buf+param_addr_offset, Param, size);
...
// quality:质检标志
int ParamTable_Read_app(void *const Param, const uint32_t _size)
{
	uint32_t size = _size;
	uint16_t addr; // 表内地址
	// 查找最后一个参数表位置
	addr = ParamTable_search_app();
	if(size>ParamTable_Size()) size=ParamTable_Size();
	// 读取整个表
	memcpy(param_buf, (const char*)(param_table_start+addr), __ParamTable_Size);
	// TEA 解密数据
	tea_decrypt(param_buf+param_addr_offset, ParamTable_Size(), param_key, param_iteration);
	memcpy(Param, (const char*)(param_buf+param_addr_offset), size);
	return size;
}
...
uint16_t param_write_key(uint32_t _key[])
{
	...
	// 用于对齐
	uint32_t Key[16];
	const struct Emb_Device_Version* const _Version = (const struct Emb_Device_Version*)0x08000200;
	for(i=0; i<param_key_len; i++)
	{
		if(0xFFFFFFFF!=_Version->signApp[i]) break;
	}
...
uint16_t param_read_key(uint32_t _key[])
{
	int i;
	const struct Emb_Device_Version* const _Version = (const struct Emb_Device_Version*)0x08000200;
	for(i=0; i<param_key_len; i++) _key[i] = _Version->signApp[i];
	return sizeof(_Version->signApp);
}
