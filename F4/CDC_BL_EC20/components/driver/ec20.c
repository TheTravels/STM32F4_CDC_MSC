...
enum ec20_resp EC20_Send(struct ec20_ofps* const _ofps, const void* const _data, const uint16_t _len)
{
	...
	while(err && (state != 0xFF))
	{
		...
		case 2:
			...
			app_debug("[%s-%d] AT+QISEND <-- : <\r\n%s\r\n>\r\n", __func__, __LINE__, _ec20_ofps._at._rbuf);
			if(1 == resp)
			{
				state = 0xFF;
				err = 0;
...
static enum ec20_resp __EC20_Read(struct ec20_ofps* const _ofps, const uint8_t socket, char* const _data, const uint16_t _len, uint16_t* const _rlen)
{
    ...
    while(err && (state != 0xFF))
    {
        ...
        //if(dlen>10240) dlen = 10240;
        //resp = at_get_resps("+QIURC: \"recv\"", NULL, NULL, 3000, 30, &_ofps->_at);
        //app_debug("[%s--%d] +QIURC[\r\n%s\r\n]\r\n", __func__, __LINE__, _ofps->_at._rbuf);
        /*if(0!=resp)
        {
        	break;
...
void EC20_Test(void)
{
	...
	//app_debug("[%s-%d] EC20_Set ...\r\n", __func__, __LINE__);
	at_print("ATI\r");
	resp = at_get_resps(ReturnOK, NULL, NULL, 1000, 100, &_ec20_ofps._at);
#if 0
	app_debug("[%s-%d] ATI <-- : <\r\n%s\r\n>\r\n", __func__, __LINE__, _ec20_ofps._at._rbuf);
	at_print("AT+QICFG=\"SEND/BUFFERSIZE\",10240\r");
	resp = at_get_resps(ReturnOK, NULL, NULL, 1000, 100, &_ec20_ofps._at);
	app_debug("[%s-%d] AT+QICFG <-- : <\r\n%s\r\n>\r\n", __func__, __LINE__, _ec20_ofps._at._rbuf);
	at_print("AT+QICFG=\"RECV/BUFFERSIZE\",10240\r");
	resp = at_get_resps(ReturnOK, NULL, NULL, 1000, 100, &_ec20_ofps._at);
	app_debug("[%s-%d] AT+QICFG <-- : <\r\n%s\r\n>\r\n", __func__, __LINE__, _ec20_ofps._at._rbuf);
#endif
	ret = EC20_Set(&_ec20_ofps);
